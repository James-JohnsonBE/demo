...
{			
	...
	function ViewModel( ) 
	{
		...
		{				    
	        ...
	        return requestID + " " + requestStatus + " " + requestSensitivity + " " + requestIntDueDate + " " + requestDueDate + " " + requestSample + " " + requestSentEmail + " " + requestAO;
	    });
		...
		self.selectedFiltersResponseTab = ko.computed(function () 
		{
	        ...
	        return requestID + " " + sampleNum + " " + responseName + " " + requestIntDueDate + " " + responseStatus + " " + responseAO + " " + responseModified;
	    });
...
/** Behaviors **/
		...
		{								
			...
			setTimeout( function()
			{
			    ...
			    if (!requestID && !requestStatus && !requestSensitivity && !requestIntDueDate && !requestDueDate && !requestSample && !requestSentEmail && !requestAO) 
				{					
				...
				{			
					...
					if( !hide && requestStatus != "" && $.trim( $(this).find(".sr1-request-status").text() ).indexOf( requestStatus ) < 0 )
						hide = true;
					if( !hide && requestSensitivity != "" && $.trim( $(this).find(".sr1-request-sensitivity").text() ).indexOf( requestSensitivity ) < 0 )
						hide = true;
					if( !hide && requestIntDueDate != "" && $.trim( $(this).find(".sr1-request-internalDueDate").text() ) != requestIntDueDate )
						hide = true;				
					if( !hide && requestDueDate != "" && $.trim( $(this).find(".sr1-request-dueDate").text() ) != requestDueDate )
						hide = true;				
		...
		{			
			...
			setTimeout( function()
			{
			    ...
			    if (!requestID && !sampleNum && !responseName && !requestIntDueDate && !responseStatus && !responseAO && !responseModified ) 
				{					
				...
				{			
					...
					if( !hide && responseName != "" && $.trim( $(this).find(".sr2-response-title").text() ) != responseName )
						hide = true;
					if( !hide && requestIntDueDate != "" && $.trim( $(this).find(".sr2-response-internalDueDate").text() ) != requestIntDueDate )
						hide = true;
					if( !hide && responseStatus != "" && $.trim( $(this).find(".sr2-response-status").text() ) != responseStatus )
						hide = true;
					if( !hide && responseAO != "" && $.trim( $(this).find(".sr2-response-ao").text() ) != responseAO )
						hide = true;
		...
		self.ClickUploadCoverSheet = function()
		{
			var oRequest = self.currentRequest();
			if( oRequest && oRequest.number )
				 m_fnUploadCoverSheet( oRequest.number );
		}
		...
		self.ClickViewResponse = function( oResponse )
		{
			...
			if( oRequest && oRequest.number && oResponse )
				m_fnViewResponse( oRequest.number, oResponse.ID, oResponse.title, oResponse.resStatus);
		}
		...
		self.ClickEditResponse = function( oResponse )
		{
			var oRequest = self.currentRequest();
			if( oRequest && oRequest.number && oResponse )
				m_fnEditResponse( oRequest.number, oResponse.ID, oResponse.title, oResponse.resStatus);
		}
		...
		self.ClickAddResponse = function()
		{
			var oRequest = self.currentRequest();
			if( oRequest && oRequest.number )
				m_fnAddResponse( oRequest.number );
		}
		...
		self.ClickUploadToResponseDocFolder = function( oResponse  )
		{
			if( oResponse && oResponse.title && oResponse.number )
		...
		self.ClickViewResponseDoc = function( oResponseDoc )
		{
			if( oResponseDoc && oResponseDoc.requestID && oResponseDoc.responseTitle )
				m_fnViewResponseDoc( oResponseDoc.ID, oResponseDoc.requestID, oResponseDoc.responseTitle );
		}
		
		self.ClickEditResponseDoc = function( oResponseDoc )
		{
			if( oResponseDoc && oResponseDoc.requestID && oResponseDoc.responseTitle )
				m_fnEditResponseDoc( oResponseDoc.ID, oResponseDoc.requestID, oResponseDoc.responseTitle );
		}
...
/** Subscriptions **/
		...
		{				
			...
			if( self.arrRequests().length > 0 && newValue ) //should trigger only once
			{
				self.arrFilteredRequestsCount( self.arrRequests().length );
				self.arrFilteredResponsesCount( self.arrResponses().length );
				
				//tab1
				ko.utils.arrayPushAll( self.ddOptionsRequestTabRequestID(), self.GetDDVals( {"type": 0, "field": "reqNumber" } ) );
				self.ddOptionsRequestTabRequestID.valueHasMutated();

				ko.utils.arrayPushAll( self.ddOptionsRequestTabRequestStatus(), self.GetDDVals( {"type": 0, "field": "status" } ) );
				self.ddOptionsRequestTabRequestStatus.valueHasMutated();

				ko.utils.arrayPushAll( self.ddOptionsRequestTabRequestInternalDueDate(), self.GetDDVals({"type": 0, "field": "internalDueDate" } ) );
				self.ddOptionsRequestTabRequestInternalDueDate.valueHasMutated();

				ko.utils.arrayPushAll( self.ddOptionsRequestTabRequestSensitivity(), self.GetDDVals({"type": 0, "field": "sensitivity" } ) );
				self.ddOptionsRequestTabRequestSensitivity.valueHasMutated();

				ko.utils.arrayPushAll( self.ddOptionsRequestTabRequestDueDate(), self.GetDDVals( {"type": 0, "field": "dueDate" } ) );
				self.ddOptionsRequestTabRequestDueDate.valueHasMutated();

				ko.utils.arrayPushAll( self.ddOptionsRequestTabRequestSample(), self.GetDDVals( {"type": 0, "field": "sample" } ) );
				self.ddOptionsRequestTabRequestSample.valueHasMutated();

				ko.utils.arrayPushAll( self.ddOptionsRequestTabRequestSentEmail(), self.GetDDVals( {"type": 0, "field": "sentEmail" } ) );
				self.ddOptionsRequestTabRequestSentEmail.valueHasMutated();
				
				ko.utils.arrayPushAll( self.ddOptionsRequestTabRequestAO(), self.GetDDVals( {"type": 0, "field" : "actionOffices", "isArr" : true } ) );
				self.ddOptionsRequestTabRequestAO.valueHasMutated();

				//tab 2
				ko.utils.arrayPushAll( self.ddOptionsResponseTabRequestID(), self.GetDDVals( {"type": 1, "field": "reqNumber" } ) );
				self.ddOptionsResponseTabRequestID.valueHasMutated();

				ko.utils.arrayPushAll( self.ddOptionsResponseTabRequestSample(), self.GetDDVals( {"type": 1, "field": "sample" } ) );
				self.ddOptionsResponseTabRequestSample.valueHasMutated();

				ko.utils.arrayPushAll( self.ddOptionsResponseTabResponseTitle(), self.GetDDVals( {"type": 1, "field": "title", "sort": true } ) );
				self.ddOptionsResponseTabResponseTitle.valueHasMutated();

				ko.utils.arrayPushAll( self.ddOptionsResponseTabRequestInternalDueDate(), self.GetDDVals( {"type": 1, "field": "internalDueDate" } ) );
				self.ddOptionsResponseTabRequestInternalDueDate.valueHasMutated();

				ko.utils.arrayPushAll( self.ddOptionsResponseTabResponseStatus(), self.GetDDVals( {"type": 1, "field": "status" } ) );
				self.ddOptionsResponseTabResponseStatus.valueHasMutated();

				ko.utils.arrayPushAll( self.ddOptionsResponseTabResponseAO(), self.GetDDVals( {"type": 1, "field": "ao" } ) );
				self.ddOptionsResponseTabResponseAO.valueHasMutated();
				
				//tab 3
				ko.utils.arrayPushAll( self.ddOptionsRequestInfoTabRequestName(), self.GetDDVals( {"type": 0, "field": "reqNumber" } ) );
				...
				ko.utils.arrayPushAll( self.ddOptionsResponseTabResponseModified(), tempArrModifed );
				ko.utils.arrayPushAll( self.ddOptionsResponseTabResponseModified(), self.GetDDVals( {"type": 1, "field": "modified", "isDate": true } ) );
				...
				setTimeout( function()
				{
					...
					else if( paramTabIndex != 1 || ( paramTabIndex == 1 && ( paramResNum == null || paramResNum == "") ) ) //dont filter here because IA has received a link to the response and we don't want the status to be filtered
		...
		self.GetDDVals = function( oObjectProperties )
		{
			...
			var types = ko.utils.arrayMap( arr, 
				function( item ) 
				{
	...
	{		
		...
		currCtx.load( m_requestItems, 'Include(ID, Title, ReqSubject, ReqStatus, IsSample, ReqDueDate, InternalDueDate, ActionOffice, EmailActionOffice, Reviewer, Owner, ReceiptDate, MemoDate, RelatedAudit, ActionItems, Comments, EmailSent, ClosedDate, ClosedBy, Modified, Sensitivity)');
	...
	function m_fnRequeryRequest( oRequest )
	{
		var currCtx = new SP.ClientContext.get_current();
		var web = currCtx.get_web();
		...
		{				
			...
			if( m_bIsSiteOwner )
			{
				...
				else //07/06/2017 - if the permissions on the request are inheriting for any reason, then reset the permissions and refresh the page
				{
					...
					for( var x = 0; x < oRequest.actionOffices.length; x++ )
					{
						...
						var sActionOfficeGroupName = Audit.Common.Utilities.GetAOSPGroupName( sActionOfficeGroupNameTitle );
						if( sActionOfficeGroupName != null && $.trim( sActionOfficeGroupName ) != "" )						
						{
							var bAOHasAccess = Audit.Common.Utilities.CheckSPItemHasGroupPermission(oRequest.item,  sActionOfficeGroupName, SP.PermissionKind.viewListItems)
							if( !bAOHasAccess )
							{
					...
					if( arrAOs != null && arrAOs.length > 0 )
					{
						...
						{											
							...
							if( actionOfficeGroup != null && actionOfficeGroup != "" )
							{
								...
								this.item.get_roleAssignments().add( actionOfficeGroup, roleDefBindingCollRestrictedRead );	
	...
	{				
					...
					for( var y = 0; y < oRequest.responses.length; y++ )
					{
						if( oRequest.responses[y]["title"] == responseID )
						{
							...
							if( responseDocObject ["checkedOutBy"] != "")
							{
								...
								oResponseDocCheckedOut["checkedOutBy"] = responseDocObject ["checkedOutBy"];								
		...
		catch( err )
		{
 		...
 		ko.utils.arrayPushAll( _myViewModel.arrResponseDocsCheckedOut(), arrResponseDocsCheckedOut ); 
		_myViewModel.arrResponseDocsCheckedOut.valueHasMutated();
	}
	...
	{		
		...
		{				
			...
			while(listItemEnumerator.moveNext())
			{
	 		...
	 		ko.utils.arrayPushAll( _myViewModel.arrCurrentRequestRequestDocs(), oRequest.requestDocs); 
			_myViewModel.arrCurrentRequestRequestDocs.valueHasMutated();
		}
		function OnFailure(sender, args)
		{
	...
	{			
		...
		{				
			...
			while(listItemEnumerator.moveNext())
			{
				...
				if( number != null )
				{
					...
					if( number == oRequest.number ) //just double checking here
					{
						...
						{							
							...
							{
								arrActionOffice.push( {"actionOffice": actionOffices[y].get_lookupValue()});
							}
						}
	 		...
	 		ko.utils.arrayPushAll( _myViewModel.arrCurrentRequestCoverSheets(), oRequest.coversheets ); 
			_myViewModel.arrCurrentRequestCoverSheets.valueHasMutated();
		}
		function OnFailure(sender, args)
		{
		...
		function RenderResponses( oRequest )
		{
			...
			for( var y = 0; y < oRequest.responses.length; y++ )
			{
				...
				for( var z = 0; z < oResponse.responseDocs.length; z++ )
				{
					...
					oResponseDoc.styleTag = Audit.Common.Utilities.GetResponseDocStyleTag2( oResponseDoc.documentStatus );
					...
					oResponseDoc.responseDocOpenInIELink = "<a target='_blank' title='Click to Open the document' onmousedown=\"return VerifyHref(this,event,'1','SharePoint.OpenDocuments','')\" " + onc + " href='" + oResponseDoc.folder + "/" + oResponseDoc.fileName + "'>" + oResponseDoc.fileName + "</a>";
					arrResponseDocs.push ( oResponseDoc );
					cnt++;
				}
				arrResponseSummaries.push( {"responseTitle" : oResponse.title,  "responseDocs" : arrResponseDocs, "responseStatus" : oResponse.resStatus, "requestStatus" : oRequest.status } );
			}
								
	 		ko.utils.arrayPushAll( _myViewModel.arrCurrentRequestResponseDocs(), arrResponseSummaries ); 		
			_myViewModel.arrCurrentRequestResponseDocs.valueHasMutated();		
		...
		for( var x = 0; x < m_arrRequests.length; x ++ )
		{
			...
			if( oRequest.status != "Closed" ) //check if all responses are closed
			{
				...
				if( oRequest.responses.length > 0 && oRequest.responses.length == countClosed )
				{
					...
					m_arrRequestsToClose.push( {number: oRequest.number, lastResponseId: lastResponseId, lastClosedDate: lastClosedDate, lastClosedBy : lastClosedBy, sLastClosedDate: sLastClosedDate, oResponse: oResponse} );
				}
			}
		}	
		
		ko.utils.arrayPushAll( _myViewModel.arrRequestsThatNeedClosing(), m_arrRequestsToClose);
		_myViewModel.arrRequestsThatNeedClosing.valueHasMutated();
	}
	...
	{		
		...
		while ( arrLength-- ) 
		{
			...
			if( m_fnIsRequestPastDue( oRequest, 0 ) )
			{
				...
				arrInternalPastDue.push( {"title" : oRequest.number, "number": oRequest.number} );
			}
			else if( m_fnIsRequestAlmostDue( oRequest, 0 ) )
			{
				...
				arrInternalAlmostDue.push( {"title" : oRequest.number, "number": oRequest.number} );
			}
			...
			if( oRequest.responses.length == 0 )
				arrRequestsWithNoResponses.push( {"title" : oRequest.number, "number": oRequest.number} );
			
			if( !oRequest.emailSent )
				arrRequestsWithNoEmailSent.push( {"title" : oRequest.number, "number": oRequest.number} );
		...
		ko.utils.arrayPushAll( _myViewModel.arrRequestsInternalAlmostDue(), arrInternalAlmostDue );
		_myViewModel.arrRequestsInternalAlmostDue.valueHasMutated();

		ko.utils.arrayPushAll( _myViewModel.arrRequestsAlmostDue(), arrAlmostDue);
		_myViewModel.arrRequestsAlmostDue.valueHasMutated();

		ko.utils.arrayPushAll( _myViewModel.arrRequestsInternalPastDue(), arrInternalPastDue);
		_myViewModel.arrRequestsInternalPastDue.valueHasMutated();

		ko.utils.arrayPushAll( _myViewModel.arrRequestsPastDue(), arrPastDue);
		_myViewModel.arrRequestsPastDue.valueHasMutated();

		ko.utils.arrayPushAll( _myViewModel.arrRequestsWithNoResponses(), arrRequestsWithNoResponses);
		_myViewModel.arrRequestsWithNoResponses.valueHasMutated();

		ko.utils.arrayPushAll( _myViewModel.arrRequestsWithNoEmailSent(), arrRequestsWithNoEmailSent);
		_myViewModel.arrRequestsWithNoEmailSent.valueHasMutated();
	}
	...
	{		
		...
		for( var x = 0; x < requestLength; x++ )
		{
			...
			{			
				...
				if( oResponse.resStatus == "2-Submitted")
					arrSubmittedResponsesByAO.push( {"title" : oResponse.title, "number": oRequest.number} );
			}	
		}
		...
		{								
			...
			$( "#" + fbody ).html( responseOutput ).show();	
		}
		_myViewModel.doSort ( true );
		
		ko.utils.arrayPushAll( _myViewModel.arrResponsesSubmittedByAO(), arrSubmittedResponsesByAO );
...
//Added 06/05/2017			
	...
	function m_fnUpdateSensitivityOnRequest( requestNumber, requestSensitivity, oldSensitivity, OnComplete )
	{
		...
		var responseDocsLib = web.get_lists().getByTitle( Audit.Common.Utilities.GetLibTitleResponseDocs() );
		var responseDocsQuery = new SP.CamlQuery();	
		responseDocsQuery.set_viewXml('<View Scope="RecursiveAll"><Query><Where><And><Eq><FieldRef Name="ReqNum"/><Value Type="Text">' + requestNumber + '</Value></Eq><Eq><FieldRef Name="ContentType"/><Value Type="Text">Document</Value></Eq></And></Where></Query></View>');
		responseDocsItems = responseDocsLib.getItems( responseDocsQuery );
		currCtx.load( responseDocsItems, 'Include(ID, ReqNum, ResID, DocumentStatus, FileLeafRef, Created )');
		...
		earesponseDocsQuery.set_viewXml('<View Scope="RecursiveAll"><Query><Where><And><Eq><FieldRef Name="RequestNumber"/><Value Type="Text">' + requestNumber + '</Value></Eq><Eq><FieldRef Name="ContentType"/><Value Type="Text">Document</Value></Eq></And></Where></Query></View>');
		earesponseDocsItems = earesponseDocsLib.getItems( earesponseDocsQuery );
		currCtx.load( earesponseDocsItems, 'Include(ID, RequestNumber, ResponseID, FileLeafRef)');

		var requestDocLib = web.get_lists().getByTitle( Audit.Common.Utilities.GetLibTitleRequestDocs() );
		var requestDocQuery = new SP.CamlQuery();	
		requestDocQuery.set_viewXml('<View Scope="RecursiveAll"><Query><Where><Eq><FieldRef Name="ReqNum"/><Value Type="Text">' + requestNumber + '</Value></Eq></Where></Query></View>');
		requestDocItems = requestDocLib.getItems( requestDocQuery );
		currCtx.load( requestDocItems, 'Include(ID, Title, ReqNum, FileLeafRef, FileDirRef)');

		var coverSheetLib = web.get_lists().getByTitle( Audit.Common.Utilities.GetLibTitleCoverSheets() );
		var coverSheetQuery = new SP.CamlQuery();	
		coverSheetQuery.set_viewXml('<View Scope="RecursiveAll"><Query><Where><Eq><FieldRef Name="ReqNum"/><Value Type="Text">' + requestNumber + '</Value></Eq></Where></Query></View>');
		coverSheetItems = coverSheetLib.getItems( coverSheetQuery );
		currCtx.load( coverSheetItems, 'Include(ID, Title, ReqNum, ActionOffice, FileLeafRef, FileDirRef)');
			...
			else
			{
				...
				while(listItemEnumerator.moveNext())
				{
					...
					if( docStatus != "Open" && docStatus != "Submitted" )
					{
						...
						var newFileName = m_fnGetNewResponseDocTitle( oListItem, responseTitle, requestSensitivity);
						...
						{
							alert( "Error occurred updating sensitivity title on Response document: " + this.curFileName + " to " + this.newFileName + " " + args.get_message() + "\n" + args.get_stackTrace());
							m_cntResponseDocsSensUpdated++;
							if( m_cntResponseDocsSensUpdated == m_cntResponseDocsSensToUpdate )
							{
								this.OnComplete( true );
						...
						var data = { OnComplete: this.OnComplete, curFileName: curFileName, newFileName: newFileName  }
					    currCtx.executeQueryAsync(Function.createDelegate(data, OnSuccessUpdateSensitivityResponseDoc), Function.createDelegate(data, OnFailureUpdateSensitivityResponseDoc));
					}
				}
				...
				while(listItemEnumerator2.moveNext())
				{
					...
					var curDocFileName = curDocFileNameAndExt.substring(0, curDocFileNameAndExt.lastIndexOf("."));
					...
					if( dateStamp.indexOf("_") >= 0 ) //then it had some sensitivity
					{
					...
					{
						alert( "Error occurred updating sensitivity title on External Auditor Response document: " + this.curFileName + " to " + this.newFileName + " " + args.get_message() + "\n" + args.get_stackTrace() );
					...
					var data = { OnComplete: this.OnComplete, curFileName: curDocFileNameAndExt, newFileName: newFileName }
				    currCtx.executeQueryAsync(Function.createDelegate(data, OnSuccessUpdateSensitivityEAResponseDoc), Function.createDelegate(data, OnFailureUpdateSensitivityEAResponseDoc));
				}
				...
				while(listItemEnumerator3.moveNext())
				{
					...
					var curDocFileNameAndExt = oListItem.get_item("FileLeafRef");
					var newFileName = m_fnGetNewFileNameForSensitivity( oListItem, oldSensitivity, requestSensitivity );
					if( newFileName != "" )
					{	
						oListItem.set_item("FileLeafRef", newFileName);
						oListItem.update();
					...
					{
						alert( "Error occurred updating sensitivity title on Request document: " + this.curFileName + " to " + this.newFileName + " " + args.get_message() + "\n" + args.get_stackTrace() );
					...
					var data = { OnComplete: this.OnComplete, curFileName: curDocFileNameAndExt, newFileName: newFileName }
				    currCtx.executeQueryAsync(Function.createDelegate(data, OnSuccessUpdateSensitivityRequestDoc), Function.createDelegate(data, OnFailureUpdateSensitivityRequestDoc));
				...
				while(listItemEnumerator4.moveNext())
				{
					...
					{
						alert( "Error occurred updating sensitivity title on Coversheet document: " + this.curFileName + " to " + this.newFileName + " " + args.get_message() + "\n" + args.get_stackTrace() );
					...
					var data = { OnComplete: this.OnComplete, curFileName: curDocFileNameAndExt, newFileName: newFileName }
				    currCtx.executeQueryAsync(Function.createDelegate(data, OnSuccessUpdateSensitivityCSDoc), Function.createDelegate(data, OnFailureUpdateSensitivityCSDoc));
				}
	    ...
	    currCtx.executeQueryAsync(Function.createDelegate(data, OnSuccessUpdateSensiLoadDocs), Function.createDelegate(data, OnFailureUpdateSensiLoadDocs));
	}
...
*/
	...
	function OnCallbackFormEditRequest(result, value)
	{
		if (result === SP.UI.DialogResult.OK) 
		{
			...
			requestItems = requestList.getItems( requestQuery );
			//request status has internal name as response status in the request list
			currCtx.load( requestItems, 'Include(ID, Title, ActionOffice, ReqStatus, Sensitivity, HasUniqueRoleAssignments, RoleAssignments, RoleAssignments.Include(Member, RoleDefinitionBindings))');
								...
								m_fnUpdateAllResponsePermissions( oListItem.get_item("ReqStatus"), m_requestNum, true, function( doneUpdatingResponses )
								{
									...
									{	
										...
										{	
											...
											m_fnUpdateSensitivityOnRequest( m_requestNum, curSensitivity, oldSensitivity, function( doneUpdatingSensitivity )
											{
...
//TODO: break these up - may cause errors		
	...
	function OnCallbackFormEditRequestDoc( result, value )
	{
		if (result === SP.UI.DialogResult.OK) 
		{
			...
			var requestDocLib = web.get_lists().getByTitle( Audit.Common.Utilities.GetLibTitleRequestDocs() );
			var requestDocQuery = new SP.CamlQuery();	
			requestDocQuery.set_viewXml('<View><Query><OrderBy><FieldRef Name="Modified" Ascending="FALSE"/></OrderBy></Query><RowLimit>1</RowLimit></View>');
			requestDocItems = requestDocLib.getItems( requestDocQuery );
			...
			(
				...
				{	
					...
					if( oListItem )
					{
						...
						var newFileName = m_fnGetNewFileNameForSensitivity( oListItem, null, requestSensitivity );
						if( newFileName != "" )
						{	
							oListItem.set_item("FileLeafRef", newFileName);
							oListItem.update();
						...
						function OnSuccessUpdateSensitivityRD(sender, args)
						{	
							m_fnRefresh();
						}		
						function OnFailureUpdateSensitivityRD(sender, args)
						{
							alert("Error updating request document name with sensitivity");									
							m_fnRefresh();
						}
						
						var data = { newFileName: newFileName }
					    currCtx.executeQueryAsync(Function.createDelegate(data, OnSuccessUpdateSensitivityRD), Function.createDelegate(data, OnFailureUpdateSensitivityRD));
	...
	function OnCallbackFormCoverSheet( result, value )
	{
		if (result === SP.UI.DialogResult.OK) 
		{
			...
			currCtx.load( coversheetItems, 'Include(ID, Title, ActionOffice, FileLeafRef, HasUniqueRoleAssignments, RoleAssignments, RoleAssignments.Include(Member, RoleDefinitionBindings))');
				...
				{	
					...
					if( oListItem )
					{
						...
						else
						{
							...
							m_fnBreakCoversheetPermissions( oListItem, false, false, function( doneBreakingCS )
							{
								if( doneBreakingCS )
								{																		
									var newFileName = m_fnGetNewFileNameForSensitivity( oListItem, null, requestSensitivity );
									...
									var data = { newFileName: newFileName }
								    currCtx.executeQueryAsync(Function.createDelegate(data, OnSuccessUpdateSensitivityCS), Function.createDelegate(data, OnFailureUpdateSensitivityCS));
	...
	function OnCallbackFormEditResponse( result, value )
	{
		...
		if (result === SP.UI.DialogResult.OK) 
		{
			...
			responseItems = responseList.getItems( responseQuery );
			currCtx.load( responseItems, 'Include(ID, Title, ActionOffice, POC, POCCC, ReturnReason, ResStatus, HasUniqueRoleAssignments, RoleAssignments, RoleAssignments.Include(Member, RoleDefinitionBindings))');
							...
							m_fnBreakRequestPermissions(oRequest.item, false, this.oListItem.get_item("ResStatus"), function( bDoneBreakingReqPermisions )
							{
								...
								if( responseDocSubmittedItems != null )
								{
									...
									while(listItemEnumerator1.moveNext())
									{
										...
										oListItem1.set_item("FileLeafRef", m_fnGetNewResponseDocTitle( oListItem1, newResponseFolderTitle, oRequest.sensitivity ));	
	...
	function BindActionOfficeHandler() //keep because jsrender on request tab needs this
	{
	...
	var publicMembers = 
	{
		...
		GoToRequest: function(requestNum, responseTitle){ m_fnGoToRequest(requestNum, responseTitle); }, //used in jsrender template
