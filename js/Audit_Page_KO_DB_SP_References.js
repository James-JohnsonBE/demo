...
<div id="tabs" style="display:none; margin-top:20px;">
	...
	<div id="tabs-0">
		<div id="lblStatusReportResponsesMsg" style="padding-top:5px;color:green;" data-bind="visible: arrResponses().length == 0"><span class='ui-icon ui-icon-info'></span>There are 0 responses for your review</div>
		<div id="divButtons" style="padding-top:3px" data-bind="visible: arrResponses().length > 0" >
			...
			<a id="btnViewAll" title="View All" href="javascript:void(0)" data-bind="visible: arrFilteredResponsesCount() < arrResponses().length, click: ClearFilters"><span class="ui-icon ui-icon-circle-zoomout"></span>View All Responses</a>
		</div>		
			...
			<table id="tblStatusReportResponses" class="tablesorter report" data-bind="visible: arrResponses().length > 0" >
				<thead>
					<tr valign="top" class="rowFilters" data-bind="visible: arrResponses().length > 0">
						<th class="sorter-false" nowrap="nowrap"><select id="ddlResponseRequestID" data-bind="options: GetDistinctResponsesDDVals('reqNumber'), value: filterRequestID, optionsCaption: '-Select-', event:{ change: FilterChanged}"></select></th>
						<th class="sorter-false" nowrap="nowrap"><select id="ddlResponseRequestStatus" data-bind="options: GetDistinctResponsesDDVals('requestStatus'), value: filterRequestStatus, optionsCaption: '-Select-', event:{ change: FilterChanged}"></select></th>
						<th class="sorter-false" nowrap="nowrap"><select id="ddlResponseRequestInternalDueDate" data-bind="options: GetDistinctResponsesDDVals('internalDueDate'), value: filterRequestIntDueDate, optionsCaption: '-Select-', event:{ change: FilterChanged}"></select></th>
						<th class="sorter-false" nowrap="nowrap"><select id="ddlResponseSampleNum" data-bind="options: GetDistinctResponsesDDVals('sample'), value: filterSampleNum, optionsCaption: '-Select-', event:{ change: FilterChanged}"></select></th>
						<th class="sorter-false" nowrap="nowrap"><select id="ddlResponseName" data-bind="options: GetDistinctResponsesDDVals('title'), value: filterResponseName, optionsCaption: '-Select-', event:{ change: FilterChanged}"></select></th>
						<th class="sorter-false" nowrap="nowrap"><select id="ddlResponseStatus" data-bind="options: GetDistinctResponsesDDVals('status'), value: filterResponseStatus, optionsCaption: '-Select-', event:{ change: FilterChanged}"></select></th>
				...
				<tbody id="fbody" data-bind="foreach: arrResponses">
					<tr class="sr-response-item" data-bind="visible: visibleRow">
						<td class="sr-response-requestNum"><span data-bind="text: reqNumber"></span></td>
						<td class="sr-response-requestStatus"><span data-bind="text: requestStatus"></span></td>
						<td class="sr-response-internalDueDate"><span data-bind="text: internalDueDate"></span></td>
						<td class="sr-response-sample"><span data-bind="text: sample"></span></td>
						<td class="sr-response-title"><a href="javascript:void(0);" title="Go to Response Details" data-bind="click: $parent.GoToResponse"><span data-bind="text: title"></span></a></td>
						<td class="sr-response-status"><span data-bind="text: status"></span></td>
						<td class="sr-response-docCount"><span data-bind="text: docCount"></span></td>
						<td class="sr-response-modified"><span data-bind="text: modified"></span></td>
					</tr>
				</tbody>
				<tfoot class="footer">
					<tr>
						<th colspan="9">Displaying <span id="spanResponsesDisplayedTotal" style="color:green" data-bind="text: arrFilteredResponsesCount()">0</span> out of <span style="color:green" data-bind="text: arrResponses().length"></span> Responses</th>
					</tr>
				</tfoot>
			</table>
		</div>
	...
	<div id="tabs-1">			
		<div style="padding-bottom:15px">
			<table>
				<tr>
					...
					<td><select id="ddlResponses" data-bind="options: GetDistinctResponsesDDVals('title'), value: filterResponseName2, optionsCaption: '-Select-'"></select></td>
				</tr>
			</table>
		</div>
				...
				<table id="tblResponseInfo" class="tablesorter">
					<tbody>
						...
						<tr>
							...
							<td><span id="requestInfoStatus" data-bind="text: $parent.currentResponseRequestStatus, style: { color: $parent.currentResponseRequestStatusStyle }"></span></td>
						</tr>
						<tr>
							<td>Subject</td>
						...
						<tr>
							...
							<td><span id="responseInfoStatus" data-bind="text: $parent.currentResponseStatus, style: { color:  resStatus == '7-Closed' ? 'red' : 'green' }"></span></td>
						</tr>
						<tr>
							<td>Sample #</td>
		...
		<div id="divResponseDocs" data-bind="visible: currentResponse">
			<fieldset><legend>Response Documents</legend>
				<div id="divEmptyResponseDocsMsg" style="border:0px !important; font-style:italic;" data-bind="visible: arrResponseDocs().length <= 0">There are 0 response documents</div>
				<table id="tblResponseDocs" class="tablesorter report" data-bind="visible: arrResponseDocs().length > 0">
					<thead>				
						<tr valign="top">
					...
					<tbody data-bind="foreach: { data: arrResponseDocs, as: 'responseDocSummary'} ">
						<tr class="requestInfo-response-doc"><td colspan="10"><img style="background-color: transparent;" src="/_layouts/images/minus.gif" title="Expand/Collapse"/><span data-bind="text: responseDocSummary.responseTitle"></span></td></tr>
						...
						<tr class="requestInfo-response-doc-item" data-bind="style: styleTag">
							...
							<td class="requestInfo-response-doc-title" title="Click to Download"><a data-bind="attr: { href: 'javascript:void(0)', onclick: link}"><span data-bind="text: title"></span></a></td>
...
<script type="text/javascript" src="../SiteAssets/js/Audit_Page_KO_DB_SP.js"></script>
