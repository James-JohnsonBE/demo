...
{				
	...
	function ViewModel( ) 
	{
		...
		{			
			if( self.arrResponses().length > 0 && self.doSort() ) //should trigger only once
			{
...
/*
	...
	{			 
	    ...
	    if( bContinue )
	    {
	    	...
	    	setTimeout( function(){ DoUpdateModel( arrResponsesToAdd ); }, 100);
	    }
	}
	...
	{	
		...
		for( var z = 0; z < oResponse.responseDocs.length; z++ )
		{
			...
			oResponseDoc ["docIcon"] =  web.mapToIcon( oResponseDoc.title, '', SP.Utilities.IconSize.Size16);
		}
		...
		{			
			...
			for( var z = 0; z < oResponse.responseDocs.length; z++ )
			{
				...
				oResponseDoc.link = 'STSNavigate(\'../_layouts/download.aspx?SourceUrl=' + oResponseDoc.folder + '/' + oResponseDoc.title + '\')';
