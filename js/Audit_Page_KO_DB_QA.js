...
{				
	...
	function ViewModel( ) 
	{
		...
		self.selectedFiltersResponseTab = ko.computed(function () 
		{
	        ...
	        return requestID + " " + requestStatus + " " + requestIntDueDate + " " + sampleNum + " " + responseName + " " + responseStatus;
	    });
...
/** Subscriptions **/
		...
		{			
			...
			if( self.arrResponses().length > 0 && newValue ) //should trigger only once
			{
				self.arrFilteredResponsesCount( self.arrResponses().length );				

				//draw these first
				ko.utils.arrayPushAll( self.ddOptionsResponseTabResponseStatus(), self.GetDDVals( "status" ) );
				self.ddOptionsResponseTabResponseStatus.valueHasMutated();

				ko.utils.arrayPushAll( self.ddOptionsResponseInfoTabResponseNameOpen2(), self.GetDDVals2( "1", true) );
				self.ddOptionsResponseInfoTabResponseNameOpen2.valueHasMutated();

				ko.utils.arrayPushAll( self.ddOptionsResponseInfoTabResponseNameProcessed2(), self.GetDDVals2( "0", true ) );
				self.ddOptionsResponseInfoTabResponseNameProcessed2.valueHasMutated();

				//draw these next				
				ko.utils.arrayPushAll( self.ddOptionsResponseTabRequestID(), self.GetDDVals( "reqNumber" ) );
				self.ddOptionsResponseTabRequestID.valueHasMutated();
				
				ko.utils.arrayPushAll( self.ddOptionsResponseTabRequestStatus(), self.GetDDVals( "requestStatus" ) );
				self.ddOptionsResponseTabRequestStatus.valueHasMutated();
				
				ko.utils.arrayPushAll( self.ddOptionsResponseTabRequestInternalDueDate(), self.GetDDVals( "internalDueDate" ) );
				self.ddOptionsResponseTabRequestInternalDueDate.valueHasMutated();

				ko.utils.arrayPushAll( self.ddOptionsResponseTabRequestSample(), self.GetDDVals( "sample" ) );
				self.ddOptionsResponseTabRequestSample.valueHasMutated();

				ko.utils.arrayPushAll( self.ddOptionsResponseTabResponseTitle(), self.GetDDVals( "title", true) );
					...
					if( paramResponseNum != null && paramResponseNum != "")
					{
						...
						{				
							if( $("#ddlResponsesOpen option[value='" + paramResponseNum + "']").length > 0)
								_myViewModel.filterResponseInfoTabResponseNameOpen2( paramResponseNum );
							else if( $("#ddlResponsesProcessed option[value='" + paramResponseNum + "']").length > 0)
								_myViewModel.filterResponseInfoTabResponseNameProcessed2( paramResponseNum );
						}
					}
		...
		self.filterResponseInfoTabResponseName = function( newValue, bOpenResponses )
		{
			...
			if( oResponse )
			{
				...
				setTimeout( function()
				{
					notifyId = SP.UI.Notify.addNotification("Displaying Response (" + oResponse.title + ")", false);
				});
			}
		}
		...
		{
			var types = ko.utils.arrayMap( self.arrResponses(), 
				function( item ) 
				{
		...
		self.GetDDVals2 = function( responseStatusType, sortAsResponse )
		{
			var types = ko.utils.arrayMap( self.arrResponses(), 
				function( item ) 
				{
					...
					if( responseStatusType == 0 ) //get the other responses
		    		{
		    		...
		    		else if( responseStatusType == 1 ) //get responses pending action
		    		{
						if( (responseStatus == "4-Approved for QA" || responseStatus == "6-Reposted After Rejection" ) && ( requestStatus == "Open" || requestStatus == "ReOpened") )  
			    			return item[ "title" ];
		    			else
	...
	{	
		...
		for( var z = 0; z < oResponse.responseDocs.length; z++ )
		{
			...
			oResponseDoc ["docIcon"] =  web.mapToIcon( oResponseDoc.fileName, '', SP.Utilities.IconSize.Size16);// Audit.Common.Utilities.GetSiteUrl() + "/" + _spPageContextInfo.layoutsUrl + "/images/" + docIcon;
		}
		...
		function RenderResponses( oResponse )
		{
			...
			for( var z = 0; z < oResponse.responseDocs.length; z++ )
			{
				...
				if( (oResponse.resStatus == "4-Approved for QA" || oResponse.resStatus == "6-Reposted After Rejection") && oResponseDoc.documentStatus == "Sent to QA" )
				{
					cntCanBeApprovedOrRejected++;
					_myViewModel.showBulkApprove( true );
				}
				else if ( (oResponse.resStatus == "4-Approved for QA" || oResponse.resStatus == "6-Reposted After Rejection")  && oResponseDoc.documentStatus == "Rejected" )
				{
					_myViewModel.showReturnToCGFS( true );
				} 					
				else if( (oResponse.resStatus == "4-Approved for QA" || oResponse.resStatus == "6-Reposted After Rejection") && ( oResponseDoc.documentStatus == "Archived" || oResponseDoc.documentStatus == "Approved" ) )
				{
			...
			var arrResponseSummary = ( {"responseTitle" : oResponse.title, "responseDocs" : arrResponseDocs, "responseStatus" : oResponse.resStatus } );
			...
			if( cntApprovedOrArchived == arrResponseDocs.length && $("#ddlResponsesOpen").val() != "" )//check if all documents are complete for QA and that if the response is still open
			{
	...
	function m_fnApproveResponseDoc( id, responseDocFileName ) 
	{
		...
		var approveResponseDocDlg = "<div id='approveResponseDocDlg' style='padding:20px; height:100px'><div style='padding:20px;'>Are you sure you would like to <span style='font-weight:bold; color:green'>Approve</span> the Response Document? <p style='padding-top:10px; font-weight:bold; color:green'>" + responseDocFileName + "</p></span></div>" + 	
			"<table style='padding-top:10px; width:200px; margin:0px auto'>" + 
				"<tr><td><input id='btnClientOk1' type='button' class='ms-ButtonHeightWidth' value='Send to Auditor' onclick='SP.UI.ModalDialog.commonModalDialogClose(SP.UI.DialogResult.OK)'/></td>" + 
				"<td class='ms-separator'>&#160;</td><td><input id='btnCancel' type='button' class='ms-ButtonHeightWidth' value='Close' onclick='SP.UI.ModalDialog.commonModalDialogClose(SP.UI.DialogResult.cancel)'/></td></tr>" + 
			"</table></div>";
	...
	function m_fnCreateEAEmailLogItem()
	{
		...
		//If folder doesn't exist, create it in EA library
		if ( !bExists ) //this should never come here
		{
	...
	function GoToResponse ( response )
	{
		...
		if( response )
		{
			...
			if( (responseStatus == "4-Approved for QA" || responseStatus == "6-Reposted After Rejection" ) && ( requestStatus  == "Open" || requestStatus == "ReOpened") )  
				...
				_myViewModel.filterResponseInfoTabResponseNameProcessed2 ( response.title );
		}
	}
