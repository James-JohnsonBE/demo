...
$(document).ready(function () {
    SP.SOD.executeFunc('sp.js', 'SP.ClientContext', ExecuteOrDelayUntilScriptLoaded(InitPermissions, "sp.js"));
...
*/
	function m_fnDisplaySPGroupPermissions( itemCollection )
	{
		...
		for( var x = 0; x < aos.length; x ++ )
		{
			...
			if( groupId != null )
			{
			    ...
			    currCtx.executeQueryAsync(Function.createDelegate(data, onLoadGroupSucceeded), Function.createDelegate(data, onLoadGroupFailed));
			}
		}
	}
	...
	function m_fnDisplaySPGroupPermissions2( itemCollection )
	{
		...
		{	
			...
			if( groupId != null )
			{
			    ...
			    currCtx.executeQueryAsync(Function.createDelegate(data, onLoadSPGroupSucceeded), Function.createDelegate(data, onLoadSPGroupFailed));
			}
		}
	}
	...
	{		
		...
		vPermGroup = window.open( location.protocol + "//" + location.host + Audit.Common.Utilities.GetSiteUrl() + "/_layouts/people.aspx?MembershipGroupId=" + groupID + "&IsDlg=1");	//pass isdlg so that it hides left nav
