...
{    
	...
	Audit.ResponseDocsReport.Report = new Audit.ResponseDocsReport.NewReportPage();
...
{			
	...
	{		
		...
		responseQuery.set_viewXml('<View><Query><OrderBy><FieldRef Name="Title"/></OrderBy><Where><Eq><FieldRef Name="ResStatus"/><Value Type="Text">1-Open</Value></Where></Eq></Query></View>');
		...
		responseDocsQuery.set_viewXml('<View Scope="RecursiveAll"><Query><Where><And><Eq><FieldRef Name="Created"/><Value Type="DateTime" IncludeTimeValue="FALSE"><Today /></Value></Eq><Eq><FieldRef Name="ContentType"/><Value Type="Text">Document</Value></Eq></And></Where></Query></View>');
		...
		currCtx.load( m_ResponseDocsItems, 'Include(ID, Title, ResID, DocumentStatus, FileLeafRef, FileDirRef, Modified, Editor)');
		...
		for( var x = 0; x < m_arrResponses.length; x++ )
		{
			...
			if( oResponse.responseDocs && oResponse.responseDocs.length > 0 )
			{
				...
				{			
					...
					var responseLink = "<a href='" + Audit.Common.Utilities.GetSiteUrl() + "/Pages/IA_DB.aspx?Tab=2&ReqNum=" + oResponse.requestNumber + "' target='_blank'>" + oResponse.title + "</a>"
					
					output += '<tr class="sr-responsedoc-item">' + 
						'<td class="sr-responsedoc-responseTitle" title="Go to Response">' + responseLink + '</td>' +
						'<td class="sr-responsedoc-docTitle">' + responseDoclink + '</td>' +
						'<td class="sr-response-downloadlink" title="Click to Download" nowrap>' + responseDocDownloadlink + '</td>' +
						'<td class="sr-response-modified" nowrap>' + oResponseDoc.modifiedDate + '</td>' +
					'</tr>';
					cnt++;
				}	
			}			
