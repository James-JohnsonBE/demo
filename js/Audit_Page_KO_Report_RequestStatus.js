...
{			
	...
	function ViewModel( ) 
	{
		...
		{			
			...
			if( self.arrRequests().length > 0 && newValue ) //should trigger only once
			{
				...
				//tab1
				ko.utils.arrayPushAll( self.ddOptionsRequestNum(), self.GetDDVals( {"type": 0, "field": "reqNumber" } ) );
				self.ddOptionsRequestNum.valueHasMutated();

				ko.utils.arrayPushAll( self.ddOptionsRequestAuditStatus(), self.GetDDVals( {"type": 0, "field": "requestAuditStatus" } ) );
				self.ddOptionsRequestAuditStatus.valueHasMutated();

				ko.utils.arrayPushAll( self.ddOptionsRequestStatus(), self.GetDDVals( {"type": 0, "field": "status" } ) );
	...
	{			
		...
		currCtx.load( m_requestItems, 'Include(ID, Title, ReqSubject, ReqStatus, IsSample, ReqDueDate, ActionOffice, ClosedDate, ClosedBy, Modified)');
	...
	function LoadRequests()
	{
		...
		try
		{
			...
			while(listItemEnumerator.moveNext())
			{
				...
				{
					curDate = new Date( curDate.getFullYear(), curDate.getMonth(), curDate.getDate()); //strip time
					if( curDate > dueDate )
					{
					...
					else
					{
						...
						if( closedDate ) //clear out the closed date so that it doesnt confuse people; date would be there if it was reopened
							closedDate = null;
					}
				}
				else if( status == "Closed" )
				{
					...
					if( closedDate )
					{
						closedDate = new Date( closedDate.getFullYear(), closedDate.getMonth(), closedDate.getDate()); //strip time
