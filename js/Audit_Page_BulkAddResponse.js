...
$(document).ready(function () {
    SP.SOD.executeFunc('sp.js', 'SP.ClientContext', ExecuteOrDelayUntilScriptLoaded(InitBulk, "sp.js"));
...
{	
	...
	{
		statusId = SP.UI.Status.addStatus("Error: Request Number was not specified. Please verify the URL Parameters or Launch from the IA Dashboard");
	...
	{					
		...
		currCtx.load( m_requestItems, 'Include(ID, Title, ReqSubject, ReqStatus, IsSample, ReqDueDate, InternalDueDate, ActionOffice)');
		...
		currCtx.load( m_responseItems, 'Include(ID, Title, ReqNum, ActionOffice, SampleNumber)' );
		...
		currCtx.load( m_ResponseDocsFoldersItems, "Include( DisplayName, Id, ContentType)");
		...
		m_bulkResponsesList = currCtx.get_web().get_lists().getByTitle( Audit.Common.Utilities.GetListNameBulkResponses() );
	...
	function m_fnLoadBulkResponses()
	{
		...
		bulkResponseQuery.set_viewXml('<View><Query><OrderBy><FieldRef Name="ID"/></OrderBy></Query></View>');
		bulkResponseItems = bulkResponesList.getItems( bulkResponseQuery );
		currCtx.load( bulkResponseItems, 'Include(ID, Title, ActionOffice, Comments, POC, POCCC)');
		...
		{						 	
			...
			$("#divLoadBulkResponsesOutput").html( output += "<tfoot><tr><th colspan='7'>Total: " + m_arrBulkResponses.length + "</tr></tfoot></table>" );
		...
		function OnFailure(sender, args)
		{
			statusId = SP.UI.Status.addStatus("Unable to load from the Bulk Response List: "  + args.get_message() + "\n" + args.get_stackTrace());
   			SP.UI.Status.setStatusPriColor(statusId, 'red');
		}		
	...
	function m_fnCreateResponses()
	{
		...
		if( confirm("Are you sure you would like to Create the Responses?" ))
		{
			...
			m_waitDialog = SP.UI.ModalDialog.showWaitScreenWithNoClose("Creating Responses", "Please wait... Creating Responses", 200, 400);
			...
			for( var x = 0; x < m_arrBulkResponses.length; x ++ )
			{
				...
				if( oBulkItem.isValid )
				{
					...
					//add action office
					var actionOfficeGroupName = Audit.Common.Utilities.GetAOSPGroupName( oBulkItem.actionOffice );
					...
					emailList = currCtx.get_web().get_lists().getByTitle( Audit.Common.Utilities.GetListTitleEmailHistory() );
					...
					{			
						...
						{
							if( !Audit.Common.Utilities.CheckIfEmailFolderExists( this.emailListFolderItems, this.requestNumber ) )
					    	{
								Audit.Common.Utilities.CreateEmailFolder( this.emailList, this.requestNumber );
					    	}
					    	m_bCreateRequestFolder = false;
				    	}


						$("#tableRow" + this.tableRowId).attr("style", "background-color:palegreen");
						$("#tableRow" + this.tableRowId).attr("title", "Created");
						$("#tdReq" + this.tableRowId).html( "<span class='ui-icon ui-icon-check'></span> " +  $("#tdReq" + this.tableRowId).text() );
					...
					function OnFailure(sender, args)
					{
						$("#tableRow" + this.tableRowId).attr("style", "background-color:salmon");
					...
					var data = {responseTitle : responseTitle, requestNumber: m_oRequest.number, tableRowId: x, emailList: emailList, emailListFolderItems: emailListFolderItems}
