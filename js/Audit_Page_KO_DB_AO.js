			...
			if( self.arrResponses().length > 0 && newValue ) //should trigger only once
			{
				...
				setTimeout( function()
				{
					...
					if( paramTabIndex == null || paramTabIndex == "" || paramTabIndex == 0 )
					{
						...
						{				
							SP.UI.Notify.addNotification("<div style='text-align:left'>There are <b>" + self.cntPendingReview() + "</b> Responses pending your review/action. <br/> <br/> Please click on the links in the <b>Response Name</b> column of the <b>Status Report tab</b> <br/> to access each response and upload documents and submit the package.</div>", false);
						}
					}
	...
	{	
		...
		function RenderResponses( oResponse )
		{
			...
			for( var z = 0; z < oResponse.responseDocs.length; z++ )
			{
				...
				if( oResponseDoc.documentStatus == "Open" && ( oResponse.resStatus == m_responseStatus1 || oResponse.resStatus == m_responseStatus2 ) )
					cntAddedByAO ++;
			...
			if( oResponse.resStatus == m_responseStatus2 && oResponse.returnReason != null && oResponse.returnReason != "" )
			{	
