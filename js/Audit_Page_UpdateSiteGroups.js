...
{		
	...
	{					
		...
		m_bulkPermissionsList = currCtx.get_web().get_lists().getByTitle( Audit.Common.Utilities.GetListNameBulkPermissions() );
	...
	function m_fnLoadBulkUsers()
	{
		...
		bulkPermissionsQuery.set_viewXml('<View><Query><OrderBy><FieldRef Name="Title"/></OrderBy></Query></View>');
		...
		function OnFailure(sender, args)
		{
			statusId = SP.UI.Status.addStatus("Unable to load from the Bulk Upload List: "  + args.get_message() + "\n" + args.get_stackTrace());
   			SP.UI.Status.setStatusPriColor(statusId, 'red');
		}		
					...
					{		
						...
						function OnSuccess(sender, args)
						{
							...
							else
							{
								...
								{
									$("#tblUnfoundUsers").html("<div>These users were not found and will be skipped:</div>" + curunfound )
								}
								else
									$("#tblUnfoundUsers").html( curunfound );
	
								//remove this from new users output
								m_curOutput = m_curOutput.split( "<LI>" + userDisplayName + "</LI>" ).join( "" );
			...
			if( username != null && username != "" )
			{
				for( var z = 0; z < m_arrBulkUsers.length; z++ )
				{
					...
					{					
						...
						oBulkItem.newUserNames = oBulkItem.newUserNames.split( username ).join( "" ); //for the ones that don't end in semi
					}
				}
			}
		}
	...
	{
		if( confirm("Are you sure you would like to Create or Update the SharePoint Groups? If Creating a Group, please re-run after the operation completed. If updating a Group, this action will replace ALL existing users in the Groups" ))
		{
			$("#btnCreateUsers").hide();
			...
			for( var x = 0; x < m_arrBulkUsers.length; x ++ )
			{
				...
				if( !oBulkItem.isValid ) //create the sharepoint group
				{
					m_countToCreate++;
					var currCtx = new SP.ClientContext.get_current();
					var web = currCtx.get_web();
					...
					{									
						...
						var rolDefColl = SP.RoleDefinitionBindingCollection.newObject( currCtx );  
						...
						newCreateGroup.set_owner( currCtx.get_web().get_associatedOwnerGroup() );
						...
						function OnSuccessUpdateGroupProps(sender, args)
						{
							...
							$("#tableRow" + this.tableRowId).attr("title", "Created SharePoint Group. Please run again to add users");
							$("#tdGroup" + this.tableRowId).html( "<span class='ui-icon ui-icon-check'></span> " +  $("#tdGroup" + this.tableRowId).text() );
						...
						function OnFailureUpdateGroupProps(sender, args)
						{
							...
							$("#tdGroup" + this.tableRowId).html( $("#tdGroup" + this.tableRowId).text() + " - " + args.get_message());

							if( m_countToCreate == m_countCreated )
							{
								document.body.style.cursor = 'default';
								notifyId = SP.UI.Notify.addNotification("Error occurred", false);
							}
						}
					...
					currCtx.executeQueryAsync(Function.createDelegate(data, OnSuccessCreateGroup), Function.createDelegate(data, OnFailureCreateGroup));
				}
