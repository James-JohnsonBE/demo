...
{			
				...
				{														
					for( var z = 0; z < m_arrRequests[y].responses.length; z++ )
					{
						...
						if( m_arrRequests[y].responses[z].title == responseTitle )
						{
							...
							m_fnBreakResponseAndFolderPermissions( "ReOpened", m_arrRequests[y].responses[z], false, true, false, false );
	...
	var m_countCSSPAdded = 0;
	var m_countCSSPToAdd = 0;	
			...
			for( var x = 0; x < arrActionOffice.length; x++ )
			{
				...
				if( actionOfficeGroup != null )
				{
					...
					function onUpdatedCSSPAOSucceeded() 
					{
						...
						if( m_countCSSPAdded == m_countCSSPToAdd )
						{
							if( this.OnComplete )
								this.OnComplete( true );
						}						
		...
		for( var y = 0; y < m_arrRequests.length; y++ )
		{
			if( m_arrRequests[y].number == requestNum )
			{
				for( var z = 0; z <  m_arrRequests[y].responses.length; z++ )
				{
					cntResponsesToBreak++;					
					var doneBreakingResponse = false;
					m_fnBreakResponseAndFolderPermissions(requestStatus, m_arrRequests[y].responses[z], false, bCheckStatus, false, false, function( doneBreakingResponse )
					{
