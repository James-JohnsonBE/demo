...
{	
	...
	{		
		...
		currCtx.load( m_requestItems, 'Include(ID, Request_x0020_Number, Request_x0020_Subject, Response_x0020_Status, Escalated_x003f_, Is_x0020_Sample, Request_x0020_DueDate, Internal_x0020_Due_x0020_Date, Internal_x0020_Status)');

		var responseList = web.get_lists().getByTitle( m_listTitleResponses );
		var responseQuery = new SP.CamlQuery();	
		responseQuery.set_viewXml('<View Scope="RecursiveAll"><Query><OrderBy><FieldRef Name="Request_x0020_Number"/></OrderBy></Query></View>');
		m_responseItems = responseList.getItems( responseQuery );
		currCtx.load( m_responseItems );
		
		//TODO: limit the query with field names. unable to query on response_status because it won't show the field in the item properties. not sure why
		//currCtx.load( m_responseItems, 'Include(ID, Response_Status)');
		m_reponseList = currCtx.get_web().get_lists().getByTitle( "Audit Responses" );
	...
	function LoadRequests()
	{
		...
		while(listItemEnumerator.moveNext())
		{
			...
			var internalStatus = oListItem.get_item('Internal_x0020_Status');
	...
	{				
		var zz = m_reponseList.get_schemaXml();
//		console.log( zz );
		
		$("#divTest").text( zz );
		var zzz = m_reponseList.get_fields();
		...
		while(listItemEnumerator.moveNext())
		{
			...
			//if( oListItem.get_item('Title') == "ORE-001-aTest-5")
			//{
			//asdfasdf = 0;
			//}
	...
	function UpdateTable(arr, fbody)
	{
		...
		for( var x = 0; x < arr.length; x++ )
		{
			...
			/************Requests**************/
			var requestLink = "<a title='View Request' href='javascript:void(0)' onclick='Audit.IAReport.Report.ViewRequest(" + oRequest.ID + ")'><span class='ui-icon ui-icon-search'>View Request</span></a>";
			var requestEditLink = "<a title='Edit Request' href='javascript:void(0)' onclick='Audit.IAReport.Report.EditRequest(" + oRequest.ID + ")'><span class='ui-icon ui-icon-pencil'>Edit Request</span></a>";
			...
			$('#' + fbody).append('<tr class="request">' + 
				...
				'<td class="request-internalStatus">' + oRequest.internalStatus + '</td>' +
			...
			$('#' + fbody).append('<tr class="coversheet-header">' + 
				'<td></td>' +
				'<td colspan="7" class="coversheet-header-title">Coversheets  (<span>' + oRequest.coversheets.length + '</span>)</td>' +
				'<td></td>' +
			'</tr>');
			...
			for( var y = 0; y < oRequest.coversheets.length; y++ )
			{
				...
				var coversheetEditLink = "<a title='Edit Coversheet' href='javascript:void(0)' onclick='Audit.IAReport.Report.EditCoverSheet(" + oCoversheet.ID + ")'><span class='ui-icon ui-icon-pencil'>Edit Coversheet</span></a>";
				
				$('#' + fbody).append('<tr class="coversheet-item">' + 
					'<td></td>' +
					'<td colspan="7" class="coversheet-title">' + oCoversheet.title + '</td>' +
			...
			$('#' + fbody).append('<tr class="response-header">' + 
				'<td></td>' +
				'<td colspan="7" class="response-header-title">Responses (<span>' + oRequest.responses.length + '</span>)</td>' +
				'<td class="response-header-action"><a title="Add Response" href="javascript:void(0)" onclick="Audit.IAReport.Report.AddResponse(\'' + oRequest.number + '\')"><span class="ui-icon ui-icon-plus">Add Response</span></a></td>' +
			'</tr>');
...
*/		
			for( var y = 0; y < oRequest.responses.length; y++ )
			{
				...
				var responseLink = "<a title='View Response' href='javascript:void(0)' onclick='Audit.IAReport.Report.ViewResponse(" + oResponse.ID + ")'><span class='ui-icon ui-icon-search'>View Response</span></a>";
				var responseEditLink = "<a title='Edit Response' href='javascript:void(0)' onclick='Audit.IAReport.Report.EditResponse(" + oResponse.ID + ")'><span class='ui-icon ui-icon-pencil'>Edit Response</span></a>";
				...
				var responseFolderAddLink = "<a title='Upload Response Documents' href='javascript:void(0)' onclick='Audit.IAReport.Report.UploadResponseDoc(\"" + oResponse.title  + "\")'><span class='ui-icon ui-icon-plus'>Upload Docs</span></a>";
				...
				/************Response Documents**************/
				var responseDocs = "<div>";
				for( var z = 0; z < oResponse.responseDocs.length; z++ )
				{
					var oResponseDoc = oResponse.responseDocs[z];
	
					var responseDocLink = "<a title='View Response Doc' href='javascript:void(0)' onclick='Audit.IAReport.Report.ViewResponseDoc(" + oResponseDoc.ID + ")'><span class='ui-icon ui-icon-search'>View Response Doc</span></a>";
					var responseEditLink = "<a title='Edit Response Doc' href='javascript:void(0)' onclick='Audit.IAReport.Report.EditResponseDoc(" + oResponseDoc.ID + ")'><span class='ui-icon ui-icon-pencil'>Edit Response Doc</span></a>";
					...
					responseDocs += '<div><span class="response-doc-title">' + oResponseDoc.title + '</span><span class="response-doc-action">' + responseDocLink + responseEditLink + '</span></div>';
					/*$('#' + fbody).append('<tr class="response-doc">' + 
						'<td></td>' +
						'<td colspan="7" class="response-doc-title"><ul><li>' + oResponseDoc.title + '</li></ul></td>' +
				...
				$('#' + fbody).append('<tr class="response-item">' + 
					...
					'<td colspan="3" class="response-documents ">' + responseDocs + '</td>' +
