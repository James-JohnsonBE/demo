...
`;
  ...
  function getPeopleByUsername(userName) {
    if (!_userPromiseMap.has(userName)) {
      _userPromiseMap.set(
        ...
        new Promise(async (resolve) => {
          ...
          if (!user)
            resolve(null);
          resolve(new People2(user));
        })
      );
    }
    return _userPromiseMap.get(userName);
  }
  function getSpecialPermGroups() {
    if (_specialGroupsPromise)
      return _specialGroupsPromise;
    _specialGroupsPromise = new Promise(async (resolve, reject2) => {
  ...
  var groupNameSpecialPermName1, groupNameSpecialPermName2, groupNameQA, _userPromiseMap, _specialGroupsPromise, getQAGroup, User, currentUser;
  var init_people_manager = __esm({
    "src/services/people_manager.js"() {
      ...
      _userPromiseMap = /* @__PURE__ */ new Map();
      _specialGroupsPromise = null;
      getQAGroup = () => getPeopleByUsername(groupNameQA);
...
`;
  ...
  async function addResponse(request2, response) {
    ...
    response.ReqNum.Value(request2);
    ...
    try {
      ...
      await onAddNewResponse(request2, response);
    } catch (e) {
  ...
  async function onAddNewResponse(request2, response) {
    if (!request2) {
      request2 = response.ReqNum.Value();
      await appContext.AuditRequests.LoadEntity(request2);
    }
  ...
  async function ensureResponseDocFolder(response) {
  ...
  async function ensureResponseDocFolderPermissions(request2, response, folder) {
    const task = addTask(
...
`;
  ...
  var NewRequestFormModule = class extends BaseForm {
    ...
    prepopulateRequestFields() {
      ...
      request2.Reminders.Value(defaultReminders);
...
`;
  ...
  var RequestDetailView = class {
    ...
    currentRequestResponseItems = ko.pureComputed(() => {
      ...
      return request2?.responses.sort(sortByField("sample")).map((response) => new ResponseItem(request2, response, this)) ?? [];
    });
    ...
    clickEnsureAllResponses = async () => {
      const request2 = ko.unwrap(this.request);
      const response = await appContext.AuditResponses.FindByColumnValue(
        [{ column: "ReqNumId", value: request2.ID }],
        {},
        {}
      );
      await Promise.all(
        response.results.map((response2) => onAddNewResponse(request2, response2))
      );
    };
    // ResponseDocs
...
`;
  ...
  // src/components/bulk_add_response/bulk_add_response.js
  init_application_db_context();
  init_infrastructure();

  // src/components/bulk_add_response/BulkAddResponseTemplate.js
  init_infrastructure();
  var bulkAddResponseTemplate = html4`
...
`;
  ...
  // src/components/bulk_add_response/bulk_add_response.js
  var componentName13 = "bulk-add-response-form";
  var BulkAddResponseForm = class {
    ...
    async clickSubmitResponses() {
      ...
      const insertPromises = bulkResponses.map(async (bulkResponseItem) => {
        ...
        const response = bulkResponse.toResponse(request2);
        const result = await addResponse(request2, response);
        if (result.isFailure) {
  ...
  directRegisterComponent(componentName13, {
  ...
  function ViewModel() {
    ...
    self.ClickBulkAddResponse = function() {
      var oRequest = self.currentRequest();
      if (oRequest && oRequest.number)
        m_fnBulkAddResponse(oRequest.ID);
    };
  ...
  function m_fnViewLateRequests() {
    window.open(
      Audit.Common.Utilities.GetSiteUrl() + "/SitePages/AuditReport_RequestsStatus.aspx",
      "_blank"
    );
  }
  ...
  function m_fnViewResponseDocsToday() {
    window.open(
      Audit.Common.Utilities.GetSiteUrl() + "/SitePages/AuditUnSubmittedResponseDocuments.aspx",
      "_blank"
    );
  }
  function m_fnViewReturnedDocs() {
    window.open(
      Audit.Common.Utilities.GetSiteUrl() + "/SitePages/AuditReturnedResponses.aspx",
      "_blank"
    );
  }
  ...
  async function m_fnBulkAddResponse(id2) {
    if (!m_bIsSiteOwner) {
      SP.UI.Notify.addNotification(
        "You do not have access to perform this action...",
        false
  ...
  function m_fnBulkEditResponse(id2) {
    ...
    options.url = Audit.Common.Utilities.GetSiteUrl() + "/SitePages/AuditBulkEditResponse.aspx?ReqNum=" + id2 + GetSourceUrlForForms();
    SP.UI.ModalDialog.showModalDialog(options);
  }
  ...
  function m_fnViewResponseDocFolder(title) {
    ...
    options.height = "600";
    options.dialogReturnValueCallback = OnCallbackForm;
    options.url = Audit.Common.Utilities.GetSiteUrl() + "/SitePages/AuditResponseDocs.aspx?RootFolder=" + Audit.Common.Utilities.GetSiteUrl() + "/" + Audit.Common.Utilities.GetLibNameResponseDocs() + "/" + title;
    SP.UI.ModalDialog.showModalDialog(options);
  }
  function m_fnViewEmailHistoryFolder(reqNum) {
    ...
    options.dialogReturnValueCallback = OnCallbackForm;
    options.url = Audit.Common.Utilities.GetSiteUrl() + "/SitePages/AuditEmailHistory.aspx?RootFolder=" + Audit.Common.Utilities.GetSiteUrl() + "/Lists/" + Audit.Common.Utilities.GetListNameEmailHistory() + "/" + reqNum + GetSourceUrlForForms();
    SP.UI.ModalDialog.showModalDialog(options);
  }
  ...
  async function m_fnBreakRequestPermissions(oListItem, refreshPageOnUpdate = false, responseStatus, OnComplete) {
    ...
    oListItem.get_roleAssignments().getByPrincipal(currentUser2).deleteObject();
    await new Promise((resolve, reject2) => {
  ...
  async function m_fnBreakCoversheetPermissions(oListItem, grantQARead) {
    ...
    var currCtx = new SP.ClientContext.get_current();
    var web = currCtx.get_web();
    var currentUser2 = currCtx.get_web().get_currentUser();
  ...
  async function m_fnBreakCoversheetPermissionsOnSpecialPerms(currCtx, oListItem, addSpecialPerms, refreshPageOnUpdate, OnComplete) {
    ...
    var web = currCtx.get_web();
    const currentUser2 = currCtx.get_web().get_currentUser();
    const ownerGroup = web.get_associatedOwnerGroup();
    const memberGroup = web.get_associatedMemberGroup();
    const visitorGroup = web.get_associatedVisitorGroup();
    oListItem.resetRoleInheritance();
  ...
  async function m_fnBreakResponseAndFolderPermissions(requestStatus, oResponse, refreshPageOnUpdate = false, checkStatus = false, bForceGrantSP = false, bForceRemoveSP = false) {
    ...
    oResponse.item.get_roleAssignments().getByPrincipal(currentUser2).deleteObject();
    if (oResponse.responseFolderItem)
      oResponse.responseFolderItem.get_roleAssignments().getByPrincipal(currentUser2).deleteObject();
    await new Promise((resolve, reject2) => {
  ...
  async function m_fnBreakResponseFolderPermissions(oListItemFolder, oListItemResponse, refreshPageOnUpdate, bCheckStatus, OnComplete) {
    ...
    oListItemFolder.get_roleAssignments().getByPrincipal(currentUser2).deleteObject();
  ...
  function m_fnGrantSpecialPermissions(requestNumber) {
    ...
    if (confirm(
      ...
      oRequest.item.get_roleAssignments().getByPrincipal(currentUser2).deleteObject();
      var data2 = { oRequest };
      currCtx.executeQueryAsync(
    ...
    if (result === SP.UI.DialogResult.OK) {
      ...
      currCtx.executeQueryAsync(
        async function() {
          ...
          oListFolderItem.get_roleAssignments().getByPrincipal(currentUser2).deleteObject();
          currCtx.executeQueryAsync(
            function() {
              m_fnRefreshData();
            },
