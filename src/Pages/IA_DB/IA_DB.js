...
Audit.IAReport.NewReportPage = function () {
  ...
  function ViewModel() {
      ...
      setTimeout(function () {
        ...
        eacher.each(function () {
          ...
          if (
            !hide &&
            requestDueDate != "" &&
            $.trim($(this).find(".sr1-request-dueDate").text()) !=
              requestDueDate
          )
            hide = true;
          if (
      ...
      setTimeout(function () {
        ...
        eacher.each(function () {
          ...
          if (
            !hide &&
            responseStatus != "" &&
            $.trim($(this).find(".sr2-response-status").text()) !=
              responseStatus
          )
            hide = true;
          if (
          ...
          if (!hide && responseModified != "") {
            ...
            if (responseModified == "Last 7 Days") {
              ...
              $.trim(responseModifiedDate).indexOf(responseModified) < 0
            )
              hide = true;
          }
    ...
    self.ClickUploadCoverSheet = function () {
      var oRequest = self.currentRequest();
      if (oRequest && oRequest.number) m_fnUploadCoverSheet(oRequest.number);
    };
  ...
  function LoadInfo() {
    ...
    currCtx.load(
      m_ResponseDocsItems,
      "Include(ID, Title, ReqNum, ResID, DocumentStatus, RejectReason, ReceiptDate, FileLeafRef, FileDirRef, File_x0020_Size, CheckoutUser, Modified, Editor)"
    );
      ...
      while (listItemEnumerator.moveNext()) {
        ...
        if (number != null) {
          ...
          if (responseObject["resStatus"] != "7-Closed")
            oRequestBigMap.responsesOpenCnt =
              oRequestBigMap.responsesOpenCnt + 1; //	m_arrRequests[x].responsesOpenCnt = m_arrRequests[x].responsesOpenCnt + 1;
      ...
      if (m_ResponseDocsFoldersItems != null) {
        ...
        while (listItemEnumerator.moveNext()) {
          ...
          if (oListItem.get_item("Title") == null) {
            ...
            currCtx.executeQueryAsync(
              OnSuccessUpdateTitle,
              OnFailureUpdateTitle
            );
          }
  ...
  function m_fnUploadCoverSheet(requestNum) {
    ...
    var options = SP.UI.$create_DialogOptions();
    options.title =
      "Upload Cover Sheet/Supplemental Document to: " + requestNum;
    options.dialogReturnValueCallback = OnCallbackFormCoverSheet;
  ...
  function m_fnAddResponse(id) {
    ...
    options.url =
      Audit.Common.Utilities.GetSiteUrl() +
      "/Lists/" +
      Audit.Common.Utilities.GetListNameResponses() +
      "/" +
      formName +
      "?ReqNum=" +
      id +
      ...
      if (m_cntResponseDocsSensToUpdate == 0) {
        ...
        while (listItemEnumerator.moveNext()) {
          ...
          if (docStatus != "Open" && docStatus != "Submitted") {
            ...
            function OnSuccessUpdateSensitivityResponseDoc(sender, args) {
              ...
              if (
                m_cntResponseDocsSensUpdated == m_cntResponseDocsSensToUpdate
              ) {
                this.OnComplete(true);
              }
            }
            ...
            currCtx.executeQueryAsync(
              ...
              Function.createDelegate(
                data,
                OnFailureUpdateSensitivityResponseDoc
              )
            );
          }
        }
        ...
        while (listItemEnumerator2.moveNext()) {
          ...
          currCtx.executeQueryAsync(
            ...
            Function.createDelegate(
              data,
              OnFailureUpdateSensitivityEAResponseDoc
            )
          );
        }
...
*/
  ...
  function m_fnRenameResponsesDocs(
    responseDocsItems,
    oldRrequestNumber,
    newRequestNumber
  ) {
    ...
    if (result === SP.UI.DialogResult.OK) {
      ...
      function onSuccess() {
        ...
        while (listItemEnumerator.moveNext()) {
          ...
          if (
            m_bigMap["request-" + m_requestNum].sensitivity != curSensitivity
          ) {
      ...
      currCtx.executeQueryAsync(
        function () {
          ...
          if (oListItem) {
            ...
            currCtx.executeQueryAsync(
              Function.createDelegate(data, OnSuccessUpdateSensitivityRD),
              Function.createDelegate(data, OnFailureUpdateSensitivityRD)
            );
          }
        },
        function (sender, args) {
                ...
                function (doneBreakingCS) {
                  if (doneBreakingCS) {
                    ...
                    currCtx.executeQueryAsync(
                      Function.createDelegate(
                        data,
                        OnSuccessUpdateSensitivityCS
                      ),
                      Function.createDelegate(
                        data,
                        OnFailureUpdateSensitivityCS
                      )
                    );
                  }
                }
        ...
        function () {
          ...
          function onUpdated1Succeeded() {
            ...
            if (
              ...
              var emailSubject =
                "Please Update your Response for Request Number: " +
                m_requestNum;
              var emailText = "";
